...
def simple_console_view(keys: list, data: list, max_width=0, *column_widths: int):
    ...
    """
    laen = defaultdict(int)
    stat = defaultdict(int)

    for line in data:
        for prop in keys:
            if prop in line:
                if len(line[prop]) > laen[prop]:
                    laen[prop] = len(line[prop])
                stat[prop] += 1  # alternative stat[prop] = stat.get(prop, 0) + 1, but slower than default dict
    laen = {k: v+2 for k, v in laen.items()}
    for prop in laen.keys():
        print(f"{prop} [{stat[prop]}]{' '*laen[prop]}"[:laen[prop]], end="")

    for line in data:
        print("") # one empty line
        for prop in laen.keys():
            if prop in line:
                print(f"{line[prop]}{' '*laen[prop]}"[:laen[prop]], end="")
            else:
                print(" "*laen[prop], end="")
    #print(json.dumps(laen, indent=2))
    ...
    try:
        ...
        if 'tracknumber' in simple_tags:
            ...
            if len(simple_tags['tracknumber']) > 2:
                logging.debug(f"Tracknumber longer than 2 digits: [{simple_tags.get('artist', '?')}-{simple_tags.get('title', '?')}]")
        return simple_tags
    except mutagen.MutagenError as e:
        logging.error(f"MutagenException: {e}")
...
if __name__ == "__main__":
    ...
    for sng_file in all_files:
        if __AUDIO_FILETYPES__[str(sng_file.suffix[1:]).lower()] is not None:
            logging.debug("MAIN::found a compatible procedure")
            pure_data = __AUDIO_FILETYPES__[str(sng_file.suffix[1:]).lower()](sng_file)
        else:
            logging.debug("MAIN::no suitable procedure found, fallback")
